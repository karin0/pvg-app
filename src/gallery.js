...
function PvgGallery(props) {
  const pages = [], modal_pages = [];
  let page = [], modal_page = [], offset = 0, ha = 0, hs = 0, cnt = 0;

  const s = new Set(), sa = new Set();
    ...
    <>
      <Box pt={8} mb={-2.5} pl={3.5}>
        <Typography
          ...
          variant="body2"
        >
          Found {props.images.length} images from {s.size} artworks by {sa.size} users
        </Typography>
      </Box>
      <GalleryPagination
        pages={pages}
